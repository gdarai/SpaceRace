    ...
    for idx, itm in pairs(list) do
        if itm ~= nil then
            if(itm["name"] == item["name"])then
                if( (itm["damage"] == item["damage"]) or (itm["damage"] == -1) or (item["damage"] == -1) ) then
                    return idx
                end
            end
        end
...
function daraiLib.compareItem(stack, item)
    ...
    if(stack["name"] == item["name"])then
        if( (stack["damage"] == item["damage"]) or (item["damage"] == -1) or (stack["damage"] == -1) ) then
            return true
        end
    end
    return false
...
function daraiLib.listInventory(ic, side)
    local size = daraiLib.getInventorySize(ic, side)
    if size == nil then return {} end
    local list = {}
    for slot = 1,size do
        local stack = daraiLib.stackInSlot(ic, side, slot)
        list[#list+1] = daraiLib.copyItem(stack, false)
    end
    return list
end

-- get slot index + how many we can insert of specific items in the inventory, but just first not full slot
function daraiLib.itemSlotNotFull(ic, side, item, selection)
    local size = daraiLib.getInventorySize(ic, side)
    for slot = 1,size do
        local stack = daraiLib.stackInSlot(ic, side, slot)
    ...
    return {daraiLib.getFreeSlot(ic, side, selection), 64}
end
...
function daraiLib.getInventorySize(ic, side)
    ...
    if side == nil and ic[2] == nil then return nil end
    if side == nil then return ic[2].inventorySize() end
...
function daraiLib.moveItems(ic, sideFrom, slotFrom, sideTo, slotTo, count)
    ...
    elseif ic[2] == nil then
        ...
        if sideFrom ~= nil then
            if slotTo ~=nil then ic[2].select(slotTo) end
            ret = ic[1].suckFromSlot(sd[sideFrom], slotFrom, count)
        end
        if sideTo ~= nil then
            if slotFrom ~=nil then ic[2].select(slotFrom) end
...
-- Creates array of indice staring min and going to max
function daraiLib.range(minI, maxI)
    local out = {}
    for i=minI,maxI do out[#out+1] = i end
    return out
end

-- Transfer all items from one inventory to the other
-- If it fails it returns slot number which was first not cleared out
function daraiLib.moveAllItems(ic, sideFrom, slotListFrom, sideTo, slotListTo)
    local slotsF = slotListFrom
    local slotsT = slotListTo
    if slotsF == nil then slotsF = daraiLib.range(1, daraiLib.getInventorySize(ic, sideFrom)) end
    if slotsT == nil then slotsT = daraiLib.range(1, daraiLib.getInventorySize(ic, sideTo)) end
    for _,slot in pairs(slotsF) do
        local stack = daraiLib.stackInSlot(ic, sideFrom, slot)
        while stack ~=nil do
            local tgt = daraiLib.itemSlotNotFull(ic, sideTo, stack, slotsT)
            if tgt[1] == -1 then return slot end
            daraiLib.moveItems(ic, sideFrom, slot, sideTo, tgt[1], stack.size)
            stack = daraiLib.stackInSlot(ic, sideFrom, slot)
        end
    end
    return 0
end
